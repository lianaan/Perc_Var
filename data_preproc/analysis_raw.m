...
for par = 1:length(Subj_set)
    ...
    if eye_tracking(par)
        ...
        data.trials_completed_total = 200;
    end
    ...
    if write_psych_curve_params
        data.mu  =  pop(par).mu2.mean(1);
        data.sigma  =  pop(par).sigma2.mean(1);
        data.lambda  =  pop(par).lambda2.mean(1);
    end
    alldata(par).cond_Ori  =  data;
    ...
    if write_psych_curve_params
        data.mu  =  pop(par).mu2.mean(3); %ORI is 1, ORI SW is 2
        data.sigma  =  pop(par).sigma2.mean(3);
        data.lambda  =  pop(par).lambda2.mean(3);
    end
    alldata(par).cond_Col  =  data;
    ...
    if eye_tracking(par)
        ...
        data = data(1:200);  %need to do this bc sometimes they are followed by nan
    end
    ...
    if write_psych_curve_params
        data.mu  =  pop(par).mu2.mean(2); %ORI is 1, ORI SW is 2
        data.sigma  =  pop(par).sigma2.mean(2);
        data.lambda  =  pop(par).lambda2.mean(2);
    end
    alldata(par).cond_OriS  =  data;
    ...
    if write_psych_curve_params
        data.mu  =  pop(par).mu2.mean(4); %ORI is 1, ORI SW is 2
        data.sigma  =  pop(par).sigma2.mean(4);
        data.lambda  =  pop(par).lambda2.mean(4);
    end
    alldata(par).cond_ColS  =  data;

function plot_TIMO_Supp(fig_ind,rec_par,ind_ctrl, ind_adhd, resp_types_all,irrel_buttons, irrel_buttons_div)
...
irrel_buttons_divR = [squeeze(irrel_buttons_div(:,1,1:2)) ...
    squeeze(irrel_buttons_div(:,2,1:4)) ...
    squeeze(irrel_buttons_div(:,3,1:2)) ...
    squeeze(irrel_buttons_div(:,4,1:4))];


irrel_buttons_m_div_ctrl=median(irrel_buttons_divR(ind_ctrl,:),1);
irrel_buttons_m_div_adhd=median(irrel_buttons_divR(ind_adhd,:),1);
...
for kk = 1:nboot
    for jj = 1:12
        ...
        sample=randsample(irrel_buttons_divR(ind_ctrl,jj),length(ind_ctrl),1);
        IB_div_ctrl(kk,jj)=median(sample);
        
        sample2=randsample(irrel_buttons_divR(ind_adhd,jj),length(ind_adhd),1);
...
%prop_corr = squeeze(resp_types_all(1:40,:,1))./(squeeze(resp_types_all(1:40,:,1))+squeeze(resp_types_all(1:40,:,2)));
...
%pa=plot([1 2 3 4], squeeze(resp_types_all(21:40,:,1))./(squeeze(resp_types_all(21:40,:,1))+squeeze(resp_types_all(21:40,:,2))), '-o','MarkerSize', 1.7*msz,'MarkerFaceColor', redd,'MarkerEdgeColor', redd, 'Color', redd); hold on;
%pc=plot([1 2 3 4], squeeze(resp_types_all(1:20,:,1))./(squeeze(resp_types_all(1:20,:,1))+squeeze(resp_types_all(1:20,:,2))), '-o','MarkerSize', msz,'MarkerFaceColor',bluee,  'Color', bluee); hold on;
...
%load('resp_types_original.mat')
resp_types_original = resp_types_all;
resp_types_all(:,:,[5 6]) = resp_types_original(:,:,[3 4]);
resp_types_all(:,:,[3 4]) = resp_types_original(:,:,[5 6]);

bar_valz_all_ctrl=[ irrel_buttons_m_ctrl(1)*sum(squeeze(resp_types_all(ind_ctrl_rec,1,3:8)),1)/sum(sum(squeeze(resp_types_all(ind_ctrl_rec,1,3:8)),1)); ...
    ...
    irrel_buttons_m_ctrl(2)*sum(squeeze(resp_types_all(ind_ctrl_rec,2,3:8)),1)/sum(sum(squeeze(resp_types_all(ind_ctrl_rec,2,3:8)),1)); ... %];
    
    irrel_buttons_m_ctrl(3)*sum(squeeze(resp_types_all(ind_ctrl_rec,3,3:8)),1)/sum(sum(squeeze(resp_types_all(ind_ctrl_rec,3,3:8)),1));...
    
    irrel_buttons_m_ctrl(4)*sum(squeeze(resp_types_all(ind_ctrl_rec,4,3:8)),1)/sum(sum(squeeze(resp_types_all(ind_ctrl_rec,4,3:8)),1));...
    ];


bar_valz_all_adhd=[ ...
    irrel_buttons_m_adhd(1)*sum(squeeze(resp_types_all(ind_adhd_rec,1,3:8)),1)/sum(sum(squeeze(resp_types_all(ind_adhd_rec,1,3:8)),1));...
    
    irrel_buttons_m_adhd(2)*sum(squeeze(resp_types_all(ind_adhd_rec,2,3:8)),1)/sum(sum(squeeze(resp_types_all(ind_adhd_rec,2,3:8)),1));...
    
    irrel_buttons_m_adhd(3)*sum(squeeze(resp_types_all(ind_adhd_rec,3,3:8)),1)/sum(sum(squeeze(resp_types_all(ind_adhd_rec,3,3:8)),1));...
    
    irrel_buttons_m_adhd(4)*sum(squeeze(resp_types_all(ind_adhd_rec,4,3:8)),1)/sum(sum(squeeze(resp_types_all(ind_adhd_rec,4,3:8)),1))];
...
box off
%l2=legend([hc(1) hc(2) hc(3) hc(4) hc(5) hc(6)], { 'space corr, feat inc, cat corr', 'space corr, feat inc, cat inc', 'space inc, feat corr, cat corr', 'space inc, feat corr, cat inc', 'space inc, feat inc, cat corr', 'space inc, feat inc, cat inc'})
...
for jj=1:12
    ...
    ij=1;
    h(ii)=bar(ii,irrel_buttons_m_div_ctrl(jj)); hold on;
    set(h(ii), 'FaceColor', colormat(ij,:), 'EdgeColor',colormat(ij,:))
    he(ii)=errorbar(ii,irrel_buttons_m_div_ctrl(jj), irrel_buttons_m_div_ctrl(jj)-bci_ib_div_ctrl(1,jj),bci_ib_div_ctrl(2,jj)-irrel_buttons_m_div_ctrl(jj) ); hold on;
...
for jj=1:12
    ...
    ij=2;
    h(ii)=bar(ii,irrel_buttons_m_div_adhd(jj)); hold on;
    set(h(ii), 'FaceColor', colormat(ij,:), 'EdgeColor',colormat(ij,:))
    he(ii)=errorbar(ii,irrel_buttons_m_div_adhd(jj), irrel_buttons_m_div_adhd(jj)-bci_ib_div_adhd(1,jj),bci_ib_div_adhd(2,jj)-irrel_buttons_m_div_adhd(jj) ); hold on;
    set(he(ii), 'Color', 'k', 'Linewidth', 1)
    errorbarT(he(ii),eb_w,eb_t)
...
%psname='responses_legendR2.pdf'
%print_pdf(psname)


%prop_corr_spatial_errors(1:40,1:4) = sum(squeeze(resp_types_all(1:40,:,[3 4 7 8])),3)./sum(squeeze(resp_types_all(1:40,:,:)),3)
